      ...
      <section className="calculatorSeoSection">
        <div className="calculatorSeoWrap">
          <div className="calculatorSeoIntro">
            <p className="label">Калькулятор профиля для ГКЛ</p>
            <h2>Онлайн-расчёт профиля для гипсокартона под объект</h2>
            <p>
              Калькулятор помогает быстро оценить расход профиля для ГКЛ для потолка, облицовки
              или перегородки. Он нужен для предварительной комплектации объекта, подготовки заявки
              и понимания примерного объёма материалов до детальной проверки проекта.
            </p>
          </div>
          ...
          <div className="calculatorSeoGrid">
            <article className="calculatorSeoCard">
              ...
              <ul>
                ...
                <li>Скачайте КП или отправьте расчёт менеджеру Иделеон.</li>
              </ul>
            </article>
            ...
            <article className="calculatorSeoCard">
              ...
              <p>
                В расчёте учитываются лист ГКЛ, профиль ПП 60×27, профиль ППН 27×28,
                стоечные профили ПС, направляющие ПН, удлинители, соединители,
                подвесы и другие элементы в зависимости от выбранной конструкции.
              </p>
            </article>

            <article className="calculatorSeoCard">
              <h3>Почему расчёт предварительный</h3>
              <p>
                На фактический расход влияют раскладка листов, высота помещения, проёмы,
                шаг стоек, усиления, потери на подрезку и требования проекта. Поэтому для
                закупки на объект лучше отправить расчёт на проверку.
              </p>
            </article>

            <article className="calculatorSeoCard">
              <h3>Когда отправить расчёт менеджеру</h3>
              <p>
                Если есть проект, ведомость материалов или большой объём поставки, отправьте
                расчёт в Иделеон. Мы проверим комплектность, уточним профиль по толщине и длине,
                подготовим коммерческое предложение и поможем с поставкой на объект.
              </p>
            </article>
          </div>
          ...
          <div className="calculatorSeoLinks">
            ...
            <div>
              <a href="/catalog/gkl-profile">Каталог профиля для ГКЛ</a>
              <a href="/articles/kak-rasschitat-profil-dlya-gipsokartona">Как рассчитать профиль для гипсокартона</a>
              <a href="/articles/tolshchina-profilya-dlya-gipsokartona">Какой толщины выбрать профиль для ГКЛ</a>
            </div>
          </div>

          <div className="calculatorFaq">
            <h3>Вопросы по расчёту профиля для ГКЛ</h3>

            <details>
              <summary>Какой запас лучше ставить в калькуляторе?</summary>
              <p>Для предварительного расчёта обычно используют запас 5–10%. Для сложных объектов, помещений с большим количеством подрезки и нестандартной геометрией запас лучше уточнять по проекту.</p>
            </details>

            <details>
              <summary>Почему профиль считается в погонных метрах?</summary>
              <p>Профиль удобнее оценивать в погонных метрах, потому что цена и закупка часто привязаны к длине профиля. В КП можно дополнительно указать выбранную длину профиля.</p>
            </details>

            <details>
              <summary>Можно ли рассчитать перегородку?</summary>
              <p>Да. В калькуляторе есть режим перегородки с выбором профилей 50, 75 и 100 мм. Для точной комплектации перегородок с проёмами и усилениями лучше прислать проект.</p>
            </details>

            <details>
              <summary>Чем отличается ПП от ПН и ПС?</summary>
              <p>ПП 60×27 чаще используется для потолков и облицовок. ПН и ППН — направляющие профили. ПС — стоечный профиль для перегородок.</p>
            </details>

            <details>
              <summary>Можно ли получить коммерческое предложение?</summary>
              <p>Да. Скачайте КП из калькулятора или отправьте расчёт в Иделеон — менеджер проверит комплектность и подготовит предложение.</p>
            </details>
          </div>
        </div>
      </section>
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "mainEntity": [
              {
                "@type": "Question",
                "name": "Какой запас лучше ставить в калькуляторе профиля для ГКЛ?",
                "acceptedAnswer": {
                  "@type": "Answer",
                  "text": "Для предварительного расчёта обычно используют запас 5–10%. Для сложных объектов, помещений с большим количеством подрезки и нестандартной геометрией запас лучше уточнять по проекту."
                }
              },
              {
                "@type": "Question",
                "name": "Почему профиль для ГКЛ считается в погонных метрах?",
                "acceptedAnswer": {
                  "@type": "Answer",
                  "text": "Профиль удобно оценивать в погонных метрах, потому что цена и закупка часто привязаны к длине профиля."
                }
              },
              {
                "@type": "Question",
                "name": "Можно ли рассчитать перегородку из ГКЛ?",
                "acceptedAnswer": {
                  "@type": "Answer",
                  "text": "Да. В калькуляторе есть режим перегородки с выбором профилей 50, 75 и 100 мм."
                }
              }
            ]
          }),
