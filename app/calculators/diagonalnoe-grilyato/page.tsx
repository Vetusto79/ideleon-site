...
export default function GrilyatoДиагональноеCalculatorPage() {
  ...
  const result = useMemo(() => {
    ...
    const latticeWithoutReserve = ceilTo(areaValue * 2.78, 1);
    const rows: ResultRow[] = [];
    rows.push(row("Решётка", "600×600 мм", null, "РГ", "шт.", 2.78, latticeWithoutReserve, reserve, 1, false, "шт."));
    rows.push(row("Профиль «мама №1»", "600 мм", 0.6, "мама №1", "шт.", 2, latticeWithoutReserve * 2, reserve, 1, true, "м.п.", true));
    rows.push(row("Профиль «мама №2»", "600 мм", 0.6, "мама №2", "шт.", 1, latticeWithoutReserve, reserve, 1, true, "м.п.", true));
    rows.push(row("Профиль «папа №1»", "600 мм", 0.6, "папа №1", "шт.", 2, latticeWithoutReserve * 2, reserve, 1, true, "м.п.", true));
    rows.push(row("Профиль «папа №2»", "600 мм", 0.6, "папа №2", "шт.", 1, latticeWithoutReserve, reserve, 1, true, "м.п.", true));
    rows.push(row("Диагональный элемент D37×15", "403 мм", 0.403, "D37×15", "шт.", 4, latticeWithoutReserve * 4, reserve, 1, true, "м.п.", true));
    rows.push(row("Обрамляющий профиль", "600 мм", 0.6, "L", "шт.", 4, latticeWithoutReserve * 4, reserve, 1, true, "м.п.", true));
    rows.push(row("Несущая направляющая L=3700 мм", "3700 мм", 3.7, "Т-15 PRIM / Т-15 GL15", "м.п.", schemeCoeff, ceilTo(areaValue * schemeCoeff, 3.7), reserve, 3.7, true, "м.п."));
    if (mountScheme === "standard") rows.push(row("Поперечная направляющая L=1200 мм", "1200 мм", 1.2, "Т-15 PRIM / Т-15 GL15", "м.п.", 1.67, ceilTo(areaValue * 1.67, 1.2), reserve, 1.2, true, "м.п."));
    rows.push(row("Поперечная направляющая L=600 мм", "600 мм", 0.6, "Т-15 PRIM / Т-15 GL15", "м.п.", schemeCoeff, ceilTo(areaValue * schemeCoeff, 0.6), reserve, 0.6, true, "м.п."));
  ...
  async function handleSend(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formData.append("message", `${offerTitle}\n${paramsText}\nФайл КП приложен.`);
  ...
  return (
    <main>
      ...
      <section className="pageHero calculatorHero">
        <Breadcrumbs items={[{ label: "Главная", href: "/" }, { label: "Калькуляторы", href: "/calculators" }, { label: "Диагональное Грильято" }]} />
        <p className="label">Калькулятор</p>
        <h1>Калькулятор Диагональное Грильято</h1>
        <p>Расчёт расхода элементов потолка Диагональное Грильято по площади, периметру и схеме монтажа.</p>
      </section>

      <section className="calculatorVisualSection">
        <div className="calculatorVisualHeader">
          <p className="label">Монтажная схема</p>
          <h2>Схема Диагональное Грильято</h2>
          <p>Схема помогает понять состав системы и основные элементы, которые участвуют в расчёте.</p>
        </div>
        <div className="calculatorVisualGrid calculatorVisualGridSingle">
          <article className="calculatorVisualCard active">
            <img src="/images/calculators/grilyato/diagonalnoe-grilyato.png" alt="Схема монтажа Диагональное Грильято" />
            <div><h3>Диагональное Грильято</h3><p>Расчёт является предварительным. Для поставки на объект лучше отправить результат менеджеру Иделеон.</p></div>
          </article>
        </div>
      </section>
      ...
      <section className="calculatorSection">
        <div className="calculatorGrid">
          <div className="calculatorPanel">
            ...
            <div className="calculatorField"><span>Схема монтажа</span><div className="calculatorTabs"><button type="button" className={mountScheme === "standard" ? "active" : ""} onClick={() => setMountScheme("standard")}>Стандартная</button><button type="button" className={mountScheme === "reinforced" ? "active" : ""} onClick={() => setMountScheme("reinforced")}>Усиленная</button></div></div>
          ...
          <div className="calculatorPanel calculatorResultPanel">
            <h2>Результат</h2>
            <div className="calculatorTableWrap"><table className="calculatorTable"><thead><tr><th>Материал</th><th>Коэффициент</th><th>Количество с запасом</th></tr></thead><tbody>{result.map((item) => (<tr key={item.element + item.catalogName}><td>{item.element}</td><td>{item.consumption}</td><td>{formatNumber(item.quantityWithReserve)} {item.unit}</td></tr>))}</tbody></table></div>
            <div className="calculatorActions"><button type="button" className="secondaryButton" onClick={handleDownload}>Скачать КП</button></div>
            <form className="calculatorSendForm" onSubmit={handleSend}>
              <h3>Отправить расчёт в Иделеон</h3>
              <p>Мы получим Excel-файл с расчётом и сможем подготовить предложение.</p>
              <input placeholder="Ваше имя" value={clientName} onChange={(e) => setClientName(e.target.value)} required />
              <input placeholder="Телефон" value={clientPhone} onChange={(e) => setClientPhone(e.target.value)} required />
              ...
              <button className="primaryButton" type="submit" disabled={sendStatus === "sending"}>{sendStatus === "sending" ? "Отправляем..." : "Отправить в Иделеон"}</button>
              {sendMessage ? <p className={sendStatus === "success" ? "formSuccess" : "formError"}>{sendMessage}</p> : null}
            </form>
            <p className="calculatorNote">Расчёт ориентировочный. На расход могут влиять раскладка, узлы примыкания, высота, шаг подвесов и требования проекта.</p>
          </div>
        </div>
      </section>
      <section className="calculatorSeoSection"><div className="calculatorSeoWrap"><div className="calculatorSeoIntro"><p className="label">Диагональное Грильято</p><h2>Расчёт потолка Диагональное Грильято</h2><p>Калькулятор рассчитывает решётку, профили «мама» и «папа», диагональный элемент, обрамление, направляющие, подвесы и уголок.</p></div></div></section>
      <SiteFooter />
    </main>
  );
}
