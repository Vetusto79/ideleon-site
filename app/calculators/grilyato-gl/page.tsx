...
const schemeLabels: Record<MountScheme, string> = {
  ...
  reinforced: "Усиленная схема",
};

const cellOptions: Record<GLType, string[]> = {
  _GL15: ["50х50", "60х60", "75х75", "86х86", "100х100", "120х120", "150х150", "200х200"],
  _GL24: ["60х60", "75х75", "86х86", "100х100", "120х120", "150х150", "200х200"],
};
...
function profileCoeff(cell: string) {
  if (cell === "50х50") return 11;
  if (cell === "60х60") return 9;
  if (cell === "75х75") return 7;
  if (cell === "86х86") return 6;
  if (cell === "100х100") return 5;
  if (cell === "120х120") return 4;
  if (cell === "150х150") return 3;
  if (cell === "200х200") return 2;
  return 0;
}

function catalogByType(type: GLType) {
  return type === "_GL15"
    ? "Т-15 PRIM / Т-15 GL15"
    : "Т-24 PRIM Line / Т-24 CLICK PRIM / Т-24 Албес Евро";
}
...
function row(
  ...
  return {
    ...
    quantityWithReserve: quantityWithoutReserve === null ? null : withReserve(quantityWithoutReserve, reservePercent, step),
    includeInOffer,
    priceBasis: priceBasis ?? (unit === "м.п." ? "м.п." : unit),
    priceByMeterPerPiece,
  };
}
...
function createExcelBlob({
  ...
  const offerRows = result.filter((item) => item.includeInOffer && item.quantityWithReserve !== null);
  const rows: string[] = [];
  ...
  rows.push(rowXml(9, [
    cell("A9", "№", "2"),
    cell("B9", "Наименование элемента", "2"),
    cell("C9", "Размер", "2"),
    cell("D9", "Длина, м", "2"),
    cell("E9", "Ед. изм.", "2"),
  ...
  offerRows.forEach((item, index) => {
    ...
    const sumFormula = item.priceByMeterPerPiece
    ...
    rows.push(rowXml(rowNumber, [
      ...
      cell(`B${rowNumber}`, item.element, "8"),
      cell(`C${rowNumber}`, item.size, "0"),
      item.lengthMeters === null ? emptyCell(`D${rowNumber}`, "4") : numberCell(`D${rowNumber}`, item.lengthMeters, "4"),
      cell(`E${rowNumber}`, item.unit, "6"),
      numberCell(`F${rowNumber}`, Number(item.quantityWithReserve ?? 0), "4"),
      cell(`G${rowNumber}`, item.priceBasis, "6"),
  ...
  rows.push(rowXml(noteRow + 1, [cell(`A${noteRow + 1}`, "Заполняйте только столбец «Цена». Столбец «Цена за» подсказывает, в каких единицах вводить стоимость.", "3")], 18));
  rows.push(rowXml(noteRow + 2, [cell(`A${noteRow + 2}`, "Для профилей с единицей «шт.» цена вводится за м.п. — Excel посчитает сумму по длине элемента и количеству.", "3")], 18));
  rows.push(rowXml(noteRow + 3, [cell(`A${noteRow + 3}`, "Расчёт ориентировочный. Точную комплектацию рекомендуется проверить по проекту.", "3")], 18));
...
export default function GrilyatoGLCalculatorPage() {
  const [area, setArea] = useState("100");
  const [perimeter, setPerimeter] = useState("40");
  const [glType, setGlType] = useState<GLType>("_GL15");
  const [cellSize, setCellSize] = useState("100х100");
  const [mountScheme, setMountScheme] = useState<MountScheme>("standard");
  ...
  const availableCells = cellOptions[glType];
  ...
  const result = useMemo(() => {
    ...
    const schemeCoeff = mountScheme === "standard" ? 0.83 : 1.67;
    const gridWithoutReserve = ceilTo(areaValue * 2.78, 1);
    const coeff = profileCoeff(cellSize);
    const catalogName = catalogByType(glType);
    const rows: ResultRow[] = [];

    rows.push(row("Решётка", "0.6×0.6", null, "РГ", "шт.", 2.78, gridWithoutReserve, reserve, 1, false, "шт."));
    rows.push(row("Профиль «мама»", "600 мм", 0.6, "мама", "шт.", coeff, gridWithoutReserve * coeff, reserve, 1, true, "м.п.", true));
    rows.push(row("Профиль «папа»", "600 мм", 0.6, "папа", "шт.", coeff, gridWithoutReserve * coeff, reserve, 1, true, "м.п.", true));
    rows.push(row("Обрамляющий профиль", "600 мм", 0.6, "L", "шт.", 4, gridWithoutReserve * 4, reserve, 1, true, "м.п.", true));
    rows.push(row("Несущая направляющая L=3700 мм", "3700 мм", 3.7, catalogName, "м.п.", schemeCoeff, ceilTo(areaValue * schemeCoeff, 3.7), reserve, 3.7, true, "м.п.", false));

    if (mountScheme === "standard") {
      rows.push(row("Поперечная направляющая L=1200 мм", "1200 мм", 1.2, catalogName, "м.п.", 1.67, ceilTo(areaValue * 1.67, 1.2), reserve, 1.2, true, "м.п.", false));
    }

    rows.push(row("Поперечная направляющая L=600 мм", "600 мм", 0.6, catalogName, "м.п.", schemeCoeff, ceilTo(areaValue * schemeCoeff, 0.6), reserve, 0.6, true, "м.п.", false));
    rows.push(row("Подвес", "по проекту", null, "АП", "шт.", schemeCoeff, ceilTo(areaValue * schemeCoeff, 10), reserve, 10, true, "шт."));
    rows.push(row("Уголок", "3000 мм", null, "PL", "м.п.", "периметр", ceilTo(perimeterValue, 3), reserve, 3, true, "м.п."));

    return rows;
  }, [area, perimeter, glType, cellSize, mountScheme, reserve]);

  function makeExcelBlob() {
    return createExcelBlob({ area, perimeter, glType, cellSize, mountScheme, reserve, result });
  }

  function downloadExcelOffer() {
    downloadBlob(makeExcelBlob(), "KP_grilyato_GL_ideleon.xlsx");
  }
  ...
  async function sendExcelOffer() {
    ...
    try {
      ...
      const file = new File([makeExcelBlob()], "KP_grilyato_GL_ideleon.xlsx", {
      ...
      formData.append("message", `Расчёт потолка Грильято GL. Тип: ${typeLabels[glType]}; ячейка: ${cellSize}; схема: ${schemeLabels[mountScheme]}; площадь: ${area} м²; периметр: ${perimeter} м; запас: ${reserve}%.`);
      formData.append("sourcePage", "/calculators/grilyato-gl");
  ...
  return (
    <main>
      ...
      <section className="pageHero calculatorHero">
        <Breadcrumbs items={[{ label: "Главная", href: "/" }, { label: "Калькуляторы", href: "/calculators" }, { label: "Грильято GL" }]} />
        <p className="label">Калькулятор</p>
        <h1>Калькулятор потолка Грильято GL</h1>
        <p>
          Онлайн-расчёт расхода элементов потолка Грильято GL: решётка, профили «мама» и «папа»,
          обрамляющий профиль, направляющие, подвесы и периметральный уголок.
        </p>
      </section>

      <section className="calculatorVisualSection">
        <div className="calculatorVisualHeader">
          <p className="label">Грильято GL</p>
          <h2>GL15, GL24 и размер ячейки</h2>
          <p>
            Визуальные карточки помогают отличить тип профиля и понять, как размер ячейки влияет
            на внешний вид потолка. Расчёт остаётся предварительным и проверяется менеджером по проекту.
          </p>
        </div>

        <div className="calculatorVisualGrid calculatorVisualGridThree">
          <article className={glType === "_GL15" ? "calculatorVisualCard active" : "calculatorVisualCard"}>
            <img src="/images/calculators/grilyato-gl/grilyato-gl15.png" alt="Грильято GL15" />
            <div>
              <h3>Грильято GL15</h3>
              <p>Более лёгкий визуальный профиль для аккуратного растрового потолка.</p>
            </div>
          </article>

          <article className={glType === "_GL24" ? "calculatorVisualCard active" : "calculatorVisualCard"}>
            <img src="/images/calculators/grilyato-gl/grilyato-gl24.png" alt="Грильято GL24" />
            <div>
              <h3>Грильято GL24</h3>
              <p>Более выраженная система с заметной шириной профиля.</p>
            </div>
          </article>

          <article className="calculatorVisualCard">
            <img src="/images/calculators/grilyato-gl/grilyato-cell-sizes.png" alt="Сравнение размеров ячеек Грильято" />
            <div>
              <h3>Размеры ячеек</h3>
              <p>Чем меньше ячейка, тем плотнее рисунок потолка и выше расход элементов.</p>
            </div>
          </article>
        </div>
      </section>
        ...
        <div className="calculatorGrid">
          <div className="calculatorPanel">
            ...
            <div className="calculatorField">
              <span>Тип грильято GL</span>
              <div className="calculatorTabs">
                {(Object.keys(typeLabels) as GLType[]).map((type) => (
                  <button
                    key={type}
                    type="button"
                    className={glType === type ? "active" : ""}
                    onClick={() => {
                      setGlType(type);
                      setCellSize(cellOptions[type].includes(cellSize) ? cellSize : cellOptions[type][0]);
                    }}
                  >
                    {typeLabels[type]}
                  </button>
            ...
            <label className="calculatorField">
              ...
              <select className="calculatorSelect" value={cellSize} onChange={(e) => setCellSize(e.target.value)}>
            ...
            <div className="calculatorField">
              <span>Схема монтажа</span>
              <div className="calculatorTabs">
                <button type="button" className={mountScheme === "standard" ? "active" : ""} onClick={() => setMountScheme("standard")}>Стандартная</button>
                <button type="button" className={mountScheme === "reinforced" ? "active" : ""} onClick={() => setMountScheme("reinforced")}>Усиленная</button>
              </div>
            </div>

            <label className="calculatorField">
              <span>Запас, %</span>
              <input value={reservePercent} onChange={(e) => setReservePercent(e.target.value)} />
            </label>

            <p className="calculatorHint">
              Формулы перенесены из расчётного файла Грильято GL. Расчёт предварительный, финальную комплектацию лучше проверить по проекту.
            </p>
          </div>
          ...
          <div className="calculatorPanel calculatorResultPanel">
                ...
                <tbody>
                  {result.map((item) => (
                    <tr key={`${item.element}-${item.catalogName}`}>
                      ...
                      <td><strong>{formatNumber(item.quantityWithReserve)}</strong> {item.unit}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="calculatorActions">
              <button type="button" className="btn secondary" onClick={downloadExcelOffer}>Скачать КП</button>
            </div>
            ...
            <p className="calculatorDisclaimer">
              В Excel-файле используется один столбец «Цена». Столбец «Цена за» подсказывает, в каких единицах вводить стоимость.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="calculatorSeoSection">
        <div className="calculatorSeoWrap">
          <div className="calculatorSeoIntro">
            <p className="label">Калькулятор Грильято GL</p>
            <h2>Расчёт потолка Грильято GL для коммерческих помещений</h2>
            <p>
              Калькулятор помогает предварительно рассчитать расход потолка Грильято GL по площади,
              периметру, типу системы, размеру ячейки и схеме монтажа. Результат можно скачать в виде КП
              или отправить менеджеру Иделеон для проверки комплектации.
            </p>
          </div>
          ...
          <div className="calculatorSeoGrid">
            <article className="calculatorSeoCard">
              ...
              <ul>
                <li>Введите площадь и периметр помещения.</li>
                <li>Выберите тип Грильято GL15 или GL24.</li>
                <li>Укажите размер ячейки и схему монтажа.</li>
                <li>Задайте запас и посмотрите расход материалов.</li>
                <li>Скачайте КП или отправьте расчёт в Иделеон.</li>
              </ul>
            </article>
            ...
            <article className="calculatorSeoCard">
              ...
              <p>
                В расчёте учитываются решётка, профили «мама» и «папа», обрамляющий профиль,
                несущие и поперечные направляющие, подвесы и периметральный уголок.
              </p>
            </article>

            <article className="calculatorSeoCard">
              <h3>От чего зависит расход</h3>
              <p>
                На количество материалов влияют площадь, периметр, выбранная ячейка, тип системы,
                стандартная или усиленная схема монтажа, запас и особенности раскладки потолка.
              </p>
            </article>
            ...
            <article className="calculatorSeoCard">
              ...
              <p>
                Менеджер проверит комплектацию, уточнит материалы по каталогу, подготовит КП,
                подскажет сроки поставки и поможет подобрать решение под объект.
              </p>
            </article>
          </div>
          ...
          <div className="calculatorSeoLinks">
            ...
            <div>
              ...
              <a href="/calculators/grilyato">Калькулятор потолка Грильято</a>
              <a href="/articles/grilyato">Статья о потолках Грильято</a>
            </div>
          </div>

          <div className="calculatorFaq">
            <h3>Вопросы по расчёту Грильято GL</h3>
            <details>
              <summary>Чем отличается GL15 от GL24?</summary>
              <p>GL15 и GL24 отличаются применяемой несущей системой и совместимыми элементами. Для точного подбора лучше проверить расчёт по проекту.</p>
            </details>
            <details>
              <summary>Что означает стандартная и усиленная схема?</summary>
              <p>Схема монтажа влияет на расход направляющих и подвесов. Усиленная схема обычно требует больше элементов каркаса.</p>
            </details>
            <details>
              <summary>Почему строка «решётка» не выводится в КП?</summary>
              <p>Решётка используется для внутренней логики расчёта, но в клиентском КП удобнее видеть отдельные элементы комплектации.</p>
            </details>
            <details>
              <summary>Можно ли получить коммерческое предложение?</summary>
              <p>Да. Скачайте КП из калькулятора или отправьте расчёт в Иделеон — менеджер проверит данные и подготовит предложение.</p>
            </details>
          </div>
        </div>
      </section>
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "mainEntity": [
              {
                "@type": "Question",
                "name": "Чем отличается Грильято GL15 от GL24?",
                "acceptedAnswer": {
                  "@type": "Answer",
                  "text": "GL15 и GL24 отличаются применяемой несущей системой и совместимыми элементами. Для точного подбора лучше проверить расчёт по проекту."
                }
              },
              {
                "@type": "Question",
                "name": "Что означает стандартная и усиленная схема монтажа Грильято GL?",
                "acceptedAnswer": {
                  "@type": "Answer",
                  "text": "Схема монтажа влияет на расход направляющих и подвесов. Усиленная схема обычно требует больше элементов каркаса."
                }
              },
              {
                "@type": "Question",
                "name": "Можно ли получить КП по расчёту Грильято GL?",
                "acceptedAnswer": {
                  "@type": "Answer",
                  "text": "Да. Калькулятор позволяет скачать КП или отправить расчёт менеджеру Иделеон для проверки и подготовки предложения."
                }
              }
            ]
          }),
