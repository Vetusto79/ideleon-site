  ...
  const bodyRows = rows
    .map((row, index) => `
      <tr>
        ...
        <td style='mso-number-format:"\\@";'>${escapeHtml(profileLengthLabel(row.profileLengthMm))}</td>
      ...
      <body>
        <table>
          <tr style="height: 92px;">
            <td colspan="8" class="brand" style="height: 92px; vertical-align: middle;">
          ...
          <tr><td colspan="8" style="height: 24px;"></td></tr>
          <tr><td colspan="8" style="height: 24px;"></td></tr>
          <tr><td colspan="8" class="title">${escapeHtml(title)}</td></tr>
          <tr><td colspan="8" class="muted">${escapeHtml(subtitle)}</td></tr>
          <tr><td colspan="8">Дата: ${date}</td></tr>
          <tr><td colspan="8">${escapeHtml(params)}</td></tr>
      ...
      <section className="calculatorSection">
        <div className="calculatorGrid">
          <div className="calculatorPanel">
            ...
            <p className="calculatorHint">Коэффициенты взяты из рабочего расчётного файла и предназначены для предварительной оценки расхода. Для точной комплектации лучше проверить расчёт по проекту.</p>
          </div>
