...
function createExcelBlob({
  ...
  rows.push(rowXml(7, [cell("A7", `Площадь: ${area} м² · периметр: ${perimeter} м · тип: ${typeLabels[grilyatoType]} · ячейка: ${cellSize} · запас: ${reserve}%`, "11")], 24));
  ...
  rows.push(rowXml(noteRow + 2, [cell(`A${noteRow + 2}`, "Для профилей цена вводится за м.п. — Excel сам посчитает сумму по длине элемента и количеству.", "3")], 18));
  ...
  return <main><SiteHeader />
    ...
    <section className="calculatorSection"><div className="calculatorGrid">
      ...
      <div className="calculatorPanel calculatorResultPanel"><h2>Результат</h2>
        ...
        <p className="calculatorDisclaimer">В Excel-файле используется один столбец «Цена». Столбец «Цена за» подсказывает, в каких единицах вводить стоимость. Оформление КП единое с калькулятором ГКЛ.</p>
      </div>
