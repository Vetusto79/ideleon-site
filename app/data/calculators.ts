...
function hiddenCassetteParams(values: Record<string, string>) {
  const scheme = values.hiddenMountScheme === "reinforced"
    ? "усиленная профилем для ГКЛ"
    : "обычная";
...
export const calculators: CalculatorConfig[] = [
  ...
  },
  {
    ...
    seoTitle: "Калькулятор закрытого кассетного потолка — обычная и усиленная системы",
    ...
    fields: [
      ...
      },
      {
        ...
        hideInput: true,
        options: [
          { label: "Обычная система", value: "simple" },
          { label: "Усиленная профилем для ГКЛ", value: "reinforced" },
        ],
      },
      { id: "module", label: "Размер кассеты", type: "buttons", defaultValue: "600×600", options: cassetteModules.map((item) => ({ label: item, value: item })) },
      cassetteMaterialField,
    ...
    visualGroups: [
      {
        ...
        description: "Обычная система использует стрингер ВТ-600 и нониусные подвесы. В усиленной добавляется отдельный каркас из профилей для ГКЛ.",
        visuals: [
          {
            title: "Обычная система",
            ...
            alt: "Монтаж кассеты закрытого потолка на обычной системе",
            diagram: "/images/calculators/cassette/cassette-hidden-simple-diagram.png",
            diagramAlt: "Техническая схема обычной закрытой системы на стрингере ВТ-600",
            fieldId: "hiddenMountScheme",
            value: "simple",
          },
          {
            title: "Усиленная профилем для ГКЛ",
    ...
    seoSections: [
      { title: "Что считает калькулятор закрытой системы", text: "Для обеих схем рассчитываются кассеты, стрингеры ВТ-600 длиной 4 м и пристенный уголок. В обычной системе дополнительно считаются верхняя и нижняя части нониусного подвеса и шплинт. В усиленной системе добавляются профили для ГКЛ ПП-1-2 и ППН-2, двухуровневые соединители, анкерные подвесы и тяги." },
      { title: "Как выбрать обычную или усиленную систему", text: "Обычная система подходит для большинства типовых помещений и использует меньше элементов. Усиленную профилем для ГКЛ выбирают при повышенной нагрузке, сложной геометрии, больших пролётах или требованиях проекта к дополнительному каркасу." },
    ...
    faq: [
      ...
      { question: "Чем усиленная схема отличается по составу?", answer: "В ней появляются профили для ГКЛ ПП-1-2 и ППН-2, двухуровневые соединители, анкерные подвесы и тяги. Обычная система использует нониусный подвес." },
